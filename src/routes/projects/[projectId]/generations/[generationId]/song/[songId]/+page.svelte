<script lang="ts">
	...
	const copyStyle = createCopyWithFeedback((copied) => { styleCopied = copied; });
	const copyLyrics = createCopyWithFeedback((copied) => { lyricsCopied = copied; });
...
<div class="flex h-full flex-col bg-gray-50 dark:bg-gray-950">
	...
	<div class="relative z-10 border-b border-gray-200/80 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
		...
		<div class="flex items-center gap-4 px-5 py-3">
			...
			<div class="group/art relative shrink-0">
				{#if generationState.song.imageUrl}
					...
					/>
				{:else}
					<div class="flex h-11 w-11 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-400 to-purple-500">
				...
				<button
					...
					class="absolute -bottom-1.5 -right-1.5 flex h-6 w-6 cursor-pointer items-center justify-center rounded-full shadow-md transition-all duration-200
						{generationState.starred
							? 'bg-amber-400 text-white shadow-amber-400/50 scale-110'
							: 'bg-white/90 text-gray-300 opacity-0 group-hover/art:opacity-100 hover:!text-amber-400 hover:!bg-white dark:bg-gray-800/90 dark:text-gray-500'}"
			...
			<div class="relative">
				...
				{#if stemState.showStemOptions}
					<div class="absolute top-11 right-0 z-20 w-64 rounded-xl border border-gray-200 bg-white p-3 shadow-xl dark:border-gray-700 dark:bg-gray-800">
						<div class="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">Separate Stems</div>
						<div class="space-y-1">
							<button
							...
							>
								...
								<div>
									<div class="text-sm font-medium text-gray-900 dark:text-gray-100">Vocals + Instrumental</div>
									<div class="text-xs text-gray-500">{#if stemState.vocalSeparation}✓ Done{:else if stemState.pendingVocalSeparation}Processing...{:else}Separate vocals{/if}</div>
								</div>
							</button>
							<button
							...
							>
								...
								<div>
									<div class="text-sm font-medium text-gray-900 dark:text-gray-100">Full Stem Split</div>
									<div class="text-xs text-gray-500">{#if stemState.stemSeparation}✓ Done{:else if stemState.pendingStemSeparation}Processing...{:else}Drums, bass, guitar…{/if}</div>
								</div>
							</button>
						</div>
					</div>
	...
	<div class="flex-1 overflow-y-auto">
		<div class="mx-auto max-w-6xl p-5">
			...
			<div class="grid min-h-96 gap-4 lg:grid-cols-[7fr_13fr]">
				...
				<div class="flex flex-col overflow-hidden rounded-2xl border border-amber-200/80 bg-gradient-to-b from-amber-50 to-amber-100 shadow-sm dark:border-amber-800/30 dark:from-amber-950/40 dark:to-amber-900/20">
					<!-- Sophisticated Notes Header -->
					<div class="flex shrink-0 items-center justify-between border-b border-amber-200/50 bg-white/50 px-6 py-4 backdrop-blur-sm dark:border-amber-800/30 dark:bg-gray-800/30">
						<div class="flex items-center gap-3">
							<div class="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-amber-400 to-amber-600">
				...
				<div class="flex min-h-0 flex-col gap-4">
					...
					{#if generationState.song.streamUrl || generationState.song.audioUrl}
						<div class="overflow-hidden rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
							<Waveform
				...
				<div class="grid min-h-0 flex-1 gap-4 {hasLyrics ? 'grid-cols-2' : 'grid-cols-1'}">
					<!-- Style -->
					<div class="group relative flex max-h-80 flex-col overflow-hidden rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
							<div class="mb-3 flex shrink-0 items-center justify-between">
								<div class="flex items-center gap-2">
									<span class="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-50 dark:bg-indigo-900/30">
							...
							<p class="min-h-0 overflow-y-auto whitespace-pre-wrap text-sm leading-relaxed text-gray-600 dark:text-gray-300">
						...
						{#if hasLyrics}
							<div class="group relative flex max-h-80 flex-col overflow-hidden rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
								<div class="mb-3 flex shrink-0 items-center justify-between">
									<div class="flex items-center gap-2">
										<span class="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-50 dark:bg-violet-900/30">
			...
			<div class="mt-4 space-y-4">
				<!-- Stem Separation -->
				{#if hasStemContent}
					<div class="overflow-hidden rounded-2xl border border-cyan-200/80 bg-gradient-to-br from-cyan-50 to-white p-5 shadow-sm dark:border-cyan-800/50 dark:from-cyan-950/30 dark:to-gray-900">
						<button
				...
				<!-- Extended Versions -->
				{#if hasExtensions}
					<div class="overflow-hidden rounded-2xl border border-green-200/80 bg-gradient-to-br from-green-50 to-white p-5 shadow-sm dark:border-green-800/50 dark:from-green-950/30 dark:to-gray-900">
						<button
							onclick={() => (extensionsCollapsed = !extensionsCollapsed)}
							class="flex w-full cursor-pointer items-center justify-between"
						>
							<h3 class="flex items-center gap-2 text-sm font-semibold text-green-900 dark:text-green-100">
								<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7" />
								</svg>
								Extended Versions
								<span class="rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-semibold text-green-700 dark:bg-green-900/50 dark:text-green-300">
									{(data.extendedGenerations ?? []).length}
								</span>
							</h3>
							<svg
								class="h-4 w-4 text-green-400 transition-transform {extensionsCollapsed ? '' : 'rotate-180'}"
								fill="none" stroke="currentColor" viewBox="0 0 24 24"
							>
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
							</svg>
