<script lang="ts">
	...
	async function handleSubmit(event: Event) {
		...
		submitError = '';
		try {
			const response = await fetch('/api/generations/upload-instrumental', {
				method: 'POST',
			...
			const payload = await response.json();
			if (!response.ok) {
				submitError = payload?.message || 'Failed to create instrumental generation';
				return;
			}
			...
			submitError = err instanceof Error ? err.message : 'Failed to create instrumental generation';
		} finally {
			isSubmitting = false;
		}
	}
...
{#if isOpen}
	<div
		...
		role="dialog"
		aria-modal="true"
		aria-labelledby="upload-instrumental-title"
		tabindex="0"
	>
		<div
		...
		>
			...
			>
				<div>
					<h2
						id="upload-instrumental-title"
						class="text-lg font-semibold text-gray-900 dark:text-gray-100"
					>
						Upload Audio to Create Instrumental
					</h2>
					<p class="text-sm text-gray-500 dark:text-gray-400">
						Upload once, keep local traceability, then generate your instrumental.
					</p>
				</div>
				<button
					type="button"
			...
			<form onsubmit={handleSubmit} class="space-y-5 p-6">
				...
				>
					<div class="flex flex-col items-center gap-3 text-center">
						<div
							class="flex h-12 w-12 items-center justify-center rounded-full bg-teal-100 text-teal-700 dark:bg-teal-900/50 dark:text-teal-300"
						>
							<svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
									...
									stroke-linejoin="round"
									stroke-width="2"
									d="M9 19V6l12-3v13M9 19a3 3 0 11-6 0 3 3 0 016 0zm12-2a3 3 0 11-6 0 3 3 0 016 0z"
								/>
							</svg>
						</div>
						...
						<div class="flex flex-wrap justify-center gap-2">
							<input
								...
								class="hidden"
								id="upload-instrumental-file"
							/>
							<label
								for="upload-instrumental-file"
								class="cursor-pointer rounded-lg bg-teal-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-teal-700"
							>
								Browse audio file
							</label>
						</div>
						{#if isUploading}
							<div
								class="h-1.5 w-full overflow-hidden rounded-full bg-teal-100 dark:bg-teal-900/50"
							>
								<div
									class="h-full w-1/2 animate-pulse bg-linear-to-r from-teal-500 to-emerald-500"
								></div>
							</div>
						{/if}
					</div>
				...
				<div class="grid gap-4 md:grid-cols-2">
					...
					<div class="md:col-span-2">
						<label
							class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300"
							for="upload-negative-tags">Negative Tags</label
						>
						<textarea
				...
				<div class="flex justify-end gap-3">
					...
					<button
						type="submit"
						disabled={!canSubmit}
						class="inline-flex items-center gap-2 rounded-lg bg-linear-to-r from-teal-600 to-emerald-600 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-teal-500/25 transition-all hover:from-teal-700 hover:to-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
					>
						{#if isSubmitting}
