	...
	let variations = $derived.by(() => {
		...
		if (annotationsContext) {
			items = items.map((item) => {
				const liveAnn = annotationsContext.get(item.annotation.generation_id, item.annotation.audio_id);
				if (liveAnn) {
	...
	<div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-3">
				<div class="flex h-10 w-10 items-center justify-center rounded-xl bg-linear-to-br from-amber-400 to-orange-500 shadow-md shadow-amber-500/20">
				...
				<div>
					...
					<p class="text-sm text-gray-500 dark:text-gray-400">
						{data.starredVariations.length} annotated variation{data.starredVariations.length !== 1 ? 's' : ''}
					</p>
				</div>
			</div>
		</div>
		...
		{#if data.starredVariations.length > 0}
			<div class="mt-4 flex gap-2">
				<button
					onclick={() => (filter = 'all')}
					class="cursor-pointer rounded-full px-3.5 py-1.5 text-xs font-medium transition-all {filter === 'all'
				...
				<button
					...
					class="cursor-pointer rounded-full px-3.5 py-1.5 text-xs font-medium transition-all {filter === 'starred'
				...
				<button
					...
					class="cursor-pointer rounded-full px-3.5 py-1.5 text-xs font-medium transition-all {filter === 'notes'
		...
		{#if variations.length === 0}
			<div class="flex flex-col items-center justify-center py-20">
				{#if data.starredVariations.length === 0}
					...
					<h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">No starred variations yet</h3>
					<p class="mt-2 max-w-sm text-center text-sm text-gray-500 dark:text-gray-400">
						Star your favorite variations or add notes from the song detail page. They'll appear here for easy access.
					</p>
				{:else}
		...
		{:else}
			<div class="mx-auto max-w-4xl space-y-3">
				{#each variations as variation (variation.annotation.id)}
					...
					{@const liveStarred = annotationsContext?.isStarred(variation.annotation.generation_id, variation.annotation.audio_id) ?? variation.annotation.starred === 1}
					<div
						class="group rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-200 hover:border-gray-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600 {isCurrentTrack ? 'ring-2 ring-indigo-500/30 border-indigo-300 dark:border-indigo-600' : ''}"
					>
						<div class="flex items-start gap-4">
							...
							>
								{#if variation.song.imageUrl}
									...
									/>
								{:else}
										<div class="flex h-full w-full items-center justify-center bg-linear-to-br from-indigo-500 to-purple-600">
										<svg class="h-7 w-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" />
										</svg>
									</div>
								{/if}
								<div class="absolute inset-0 flex items-center justify-center bg-black/0 transition-all group-hover/play:bg-black/40">
									{#if isPlaying}
									...
									{:else}
										<div class="flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-lg opacity-0 transition-opacity group-hover/play:opacity-100">
											<svg class="h-4 w-4 pl-0.5 text-gray-900" fill="currentColor" viewBox="0 0 24 24">
							...
							<div class="min-w-0 flex-1">
								<div class="flex items-start justify-between gap-2">
									<div class="min-w-0 flex-1">
										<a
											href="/projects/{variation.generation.project_id}/generations/{variation.generation.id}/song/{variation.song.id}"
										...
										<div class="mt-0.5 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
											<span class="inline-flex items-center gap-1 rounded bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300">
											...
											<span class="truncate text-xs" title={formatDate(variation.annotation.updated_at)}>
									...
									<!-- Star button -->
									<button
										onclick={() => handleToggleStar(variation.annotation.generation_id, variation.annotation.audio_id)}
									...
									>
										<svg class="h-5 w-5 {starAnimations.get(variation.annotation.audio_id) || ''}" fill={liveStarred ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
								...
								{#if isCurrentTrack}
									<div class="mt-2 flex items-center gap-2">
										...
										<div class="relative h-1.5 flex-1 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
											<div
