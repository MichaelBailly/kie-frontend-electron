<script lang="ts">
	...
	function buildStemTrack(stem: StemTrack): AudioTrack {
		return {
			id: `${separation.audio_id}:${stem.key}`,
			generationId: separation.generation_id,
			projectId,
			title: `${songTitle} — ${stem.name} Stem`,
			imageUrl,
			streamUrl: stem.url,
			audioUrl: stem.url,
			duration: null
	...
	function toggleStem(stem: StemTrack) {
		const trackId = `${separation.audio_id}:${stem.key}`;
		if (audioStore.isCurrentTrack(trackId)) {
			audioStore.toggle();
		} else {
			audioStore.play(buildStemTrack(stem));
		}
	}

	function isStemPlaying(stemKey: string): boolean {
		return audioStore.isTrackPlaying(`${separation.audio_id}:${stemKey}`);
	}

	function isStemCurrent(stemKey: string): boolean {
		return audioStore.isCurrentTrack(`${separation.audio_id}:${stemKey}`);
	}
...
<div class="space-y-4">
	...
	<div class="flex items-center justify-end gap-2">
		<button
			...
			class="flex cursor-pointer items-center gap-2 rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
	...
	<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-3 xl:grid-cols-4">
		{#each stems as stem (stem.name)}
			<div
				class="rounded-lg border p-3 transition-colors {isStemCurrent(stem.key)
			...
			>
				<div class="flex items-start gap-3">
					<button
						...
						class="flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-full transition-colors {isStemPlaying(
							stem.key
						)
							? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/25'
							: 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300'}"
					>
						{#if isStemPlaying(stem.key)}
				...
				>
					...
					{/if}
					{#if onAddVocals && stem.key === 'instrumental'}
						<button
							type="button"
							onclick={() => onAddVocals(stem.key, stem.url!)}
							class="rounded p-1.5 text-violet-500 transition-colors hover:bg-violet-100 hover:text-violet-700 dark:text-violet-400 dark:hover:bg-violet-900/30 dark:hover:text-violet-200"
							title="Add vocals from {stem.name}"
						>
							<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
									stroke-linecap="round"
