	...
	<main class="mx-auto max-w-3xl px-6 py-10">
		...
		<section class="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm">
			...
			<div class="p-6">
				...
				{#if hasExistingKey}
					<div
						class="mb-6 flex items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3"
					>
				...
				{:else}
					<div
						class="mb-6 flex items-center gap-3 rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3"
					>
				...
				<div class="space-y-4">
						...
						<div class="relative">
							<input
								...
								placeholder={hasExistingKey
									...
									: 'Enter your KIE API key...'}
								class="w-full rounded-xl border border-white/10 bg-white/5 py-3.5 pr-12 pl-4 font-mono text-sm text-white placeholder-gray-500 transition-all focus:border-indigo-500/50 focus:bg-white/10 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none"
							/>
							<button
								type="button"
								onclick={() => (showApiKey = !showApiKey)}
								class="absolute top-1/2 right-3 -translate-y-1/2 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-white/10 hover:text-gray-300"
						...
						>
							{#if validationResult.valid}
								...
								<span class="text-sm text-emerald-300"
									>{validationResult.message || 'API key is valid'}</span
								>
							{:else}
								<svg
								...
								<span class="text-sm text-red-300"
									>{validationResult.error || 'Invalid API key'}</span
								>
							{/if}
						</div>
					{/if}
					...
					{#if saveMessage}
						<div
							class="flex items-center gap-3 rounded-xl border px-4 py-3 {saveMessage.type ===
							'success'
								? 'border-emerald-500/20 bg-emerald-500/10'
								: 'border-red-500/20 bg-red-500/10'}"
						>
					...
					<div class="flex flex-wrap gap-3 pt-2">
						...
						<button
							...
							class="flex items-center gap-2 rounded-xl border border-white/20 bg-white/5 px-5 py-2.5 text-sm font-medium text-white backdrop-blur-sm transition-all hover:border-white/30 hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50"
						>
							{#if isValidating}
								<svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
									<circle
